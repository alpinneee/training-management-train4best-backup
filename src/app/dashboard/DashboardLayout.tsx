...
export default function DashboardLayout({
      ...
      if (response.ok) {
        ...
        if (data.data) {
          setDashboardData({
            ...
            certificationTypeData: data.data.certificationTypeData || certificationTypeData,
  ...
  const locale = {
    ...
    formatMonthYear: (locale: string | undefined, date: Date) => {
      return new Intl.DateTimeFormat(localeString, { month: 'long', year: 'numeric' }).format(date);
    },
    ...
    formatWeekday: (locale: string | undefined, date: Date) => {
      return new Intl.DateTimeFormat(localeString, { weekday: 'long' }).format(date);
    },
    formatShortWeekday: (locale: string | undefined, date: Date) => {
      return new Intl.DateTimeFormat(localeString, { weekday: 'short' }).format(date);
    }
  };
  ...
  const certificateCount = dashboardData.certificationTypeData.reduce((sum, item) => sum + item.value, 0);
  const trainingCount = dashboardData.upcomingTrainings.length;
  const participantCount = dashboardData.trainingData.reduce((sum, item) => sum + item.value, 0);
  const trainerCount = [...new Set(dashboardData.upcomingTrainings.map(t => t.trainer))].length;
  const programCount = [...new Set(dashboardData.upcomingTrainings.map(t => t.title))].length;
