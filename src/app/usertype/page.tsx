...
const deleteUsertypeAPI = async (
  idUsertype: string,
  force: boolean = false
): Promise<void> => {
  try {
    console.log(
      `Attempting to delete usertype: ${idUsertype}, force: ${force}`
    );
    const url = force
      ? `/api/usertype/${idUsertype}?force=true`
      : `/api/usertype/${idUsertype}`;
...
const UserPage = (): ReactElement => {
  ...
  return (
    <Layout>
      <div className="p-2">
        <h1 className="text-lg md:text-xl text-gray-700 mb-2">UserType</h1>
        ...
        {isDeleteModalOpen && (
          <Modal onClose={() => setIsDeleteModalOpen(false)}>
            ...
            <p className="text-xs text-gray-600 mt-2">
              Apakah Anda yakin ingin menghapus usertype{" "}
