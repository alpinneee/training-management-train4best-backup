...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (!userEmail) {
          try {
            const profileResponse = await fetch('/api/instructure/get?email=demo@example.com');
            if (profileResponse.ok) {
              ...
              if (profileData.data && profileData.data.email) {
                ...
                if (userEmail) {
                  ...
                  console.log('Loaded email from instructure profile:', userEmail);
                }
              }
            }
          } catch (profileError) {
            console.log('Could not load instructure profile data, using fallback email');
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      ...
      if (userEmail) {
        try {
          const profileResponse = await fetch(`/api/instructure/get?email=${encodeURIComponent(userEmail)}`);
          if (profileResponse.ok) {
            const profileData = await profileResponse.json();
            if (profileData.data) {
              // Store profile data in localStorage for consistency
              localStorage.setItem('instructureProfile', JSON.stringify(profileData.data));
              console.log('Instructure profile data loaded and stored:', profileData.data);
            }
          }
        } catch (error) {
          console.log('Could not load instructure profile data:', error);
        }
      }
    };
