...
import { FileText, Edit, Trash2, History, UserCheck, Award, FileSignature, BarChart, Info, XCircle, Trash, BookOpen } from "lucide-react";
...
const CourseScheduleDetail = () => {
  ...
  const [selectedValueForEdit, setSelectedValueForEdit] = useState<{id: string, valueType: string, remark: string, value: string} | null>(null);
  ...
  const fetchCourseMaterials = async () => {
    ...
    try {
      const response = await fetch(`/api/course-schedule/${scheduleId}/material`);
  ...
  const handleDeleteValue = async (valueId: string) => {
    if (confirm('Are you sure you want to delete this value?')) {
      ...
      setValueError(null);
      try {
        const response = await fetch(`/api/course-schedule/${scheduleId}/participant/value/${valueId}`, {
          method: 'DELETE'
        });
        ...
        if (selectedParticipantForValue) {
          await fetchParticipantValues(selectedParticipantForValue.participantId, valuePage);
          toast.success("Value deleted successfully");
        }
      } catch (error) {
        console.error('Error deleting value:', error);
        setValueError(error instanceof Error ? error.message : 'Failed to delete value');
  ...
  const handleEditValueInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    if (selectedValueForEdit) {
      setSelectedValueForEdit(prev => prev ? {
        ...prev,
        [name]: value
      } : null);
    }
  ...
  const handleEditValueSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/course-schedule/${scheduleId}/participant/value/${selectedValueForEdit.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      setEditValueError(error instanceof Error ? error.message : 'Failed to update value');
    } finally {
        ...
        <div>
          ...
          <div className="overflow-x-auto -mx-2 px-2">
            <table className="w-full bg-white rounded-lg shadow-sm">
              ...
              <tbody>
                {courseDetails.instructures && courseDetails.instructures.length > 0 ? (
        ...
        <Modal onClose={() => {
          ...
          <div className="p-2">
            ...
            <div className="bg-gray-50 p-3 rounded-lg mb-4">
              <h3 className="text-sm font-medium mb-2 text-gray-700">Add New Value</h3>
                  ...
                  <table className="w-full bg-white rounded-lg shadow-sm">
                    <thead>
                      <tr className="border-b">
                        ...
                        <th className="text-left p-2 text-xs text-gray-700 w-24">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {valueData.length > 0 ? (
        ...
        <div className="p-2">
          ...
          <form onSubmit={handleEditValueSubmit}>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-600 mb-1">Value Type *</label>
                <input
                  type="text"
                  name="valueType"
      ...
      {detailLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 border-b pb-4">
            ...
            <div>
              <h3 className="font-medium text-gray-700">{detailData?.personalInfo?.name}</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <p className="text-gray-700">{detailData?.personalInfo?.name}</p>
              </div>
              <div>
                <p className="text-gray-500">Email</p>
                <p className="text-gray-700">{detailData?.personalInfo?.email}</p>
              </div>
              <div>
                <p className="text-gray-500">Phone</p>
                <p className="text-gray-700">{detailData?.personalInfo?.phone}</p>
              </div>
              <div>
                <p className="text-gray-500">Address</p>
                <p className="text-gray-700">{detailData?.personalInfo?.address}</p>
              </div>
            </div>
          </div>
          ...
          {/* Course Information */}
          <div>
            <h3 className="font-medium mb-2 text-gray-700">Course Information</h3>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <p className="text-gray-500">Present Days</p>
                <p className="text-gray-700">{detailData?.courseInfo?.presentDays}</p>
              </div>
              <div>
                <p className="text-gray-500">Total Days</p>
                <p className="text-gray-700">{detailData?.courseInfo?.totalDays}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-gray-700">{detailData?.courseInfo?.regStatus}</p>
              </div>
              <div>
                <p className="text-gray-500">Joined Date</p>
                <p className="text-gray-700">{detailData?.courseInfo?.joinedDate ? new Date(detailData.courseInfo.joinedDate).toLocaleDateString() : ''}</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <p className="text-gray-500">Amount Paid</p>
                  <p className="text-gray-700">{detailData?.courseInfo?.payment?.amount?.toLocaleString()} Rp</p>
              </div>
              <div>
                <p className="text-gray-500">Total Price</p>
                <p className="text-gray-700">{detailData?.courseInfo?.payment?.total?.toLocaleString()} Rp</p>
              </div>
              <div>
                <p className="text-gray-500">Payment Method</p>
                <p className="text-gray-700">{detailData?.courseInfo?.payment?.method}</p>
              </div>
              <div>
                <p className="text-gray-500">Payment Date</p>
                <p className="text-gray-700">{detailData?.courseInfo?.payment?.date ? new Date(detailData.courseInfo.payment.date).toLocaleDateString() : 'Not paid yet'}</p>
              </div>
            </div>
          </div>
          ...
          {/* Certificate */}
          <div>
            <h3 className="font-medium mb-2 text-gray-700">Certificate</h3>
            {detailData?.certificateInfo ? (
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <p className="text-gray-500">Certificate Number</p>
                  <p className="text-gray-700">{detailData.certificateInfo.number}</p>
                </div>
                <div>
                  <p className="text-gray-500">Issue Date</p>
                  <p className="text-gray-700">{new Date(detailData.certificateInfo.issueDate).toLocaleDateString()}</p>
                </div>
              </div>
            ) : (
          ...
          {/* Test Results */}
          <div>
            <h3 className="font-medium mb-2 text-gray-700">Test Results</h3>
            {detailData?.testResults && detailData.testResults.length > 0 ? (
              <div className="space-y-2">
                {detailData.testResults.map((test: {id: string, type: string, remark: string, value: number, maxValue: number}) => (
                  <div key={test.id} className="text-xs">
                    <div className="flex justify-between mb-1">
                      <span className="text-gray-700">{test.type} {test.remark ? `(${test.remark})` : ''}</span>
                      <span className="font-medium text-gray-700">{test.value} / {test.maxValue}</span>
                    </div>
