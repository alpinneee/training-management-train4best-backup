...
const CourseScheduleDetail = () => {
  ...
  const handleOpenEditAttendance = (att: AttendanceData) => { 
    setEditAttendanceData({ ...att });
    setIsEditAttendanceModalOpen(true);
  };
  // Handler buka modal delete
  const handleOpenDeleteAttendance = (id: string) => {
  ...
  const handleUpdateAttendance = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editAttendanceData) return; // Add this line
    ...
    try {
      ...
      if (instructureCertificateData.pdfFile) {
        try {
          ...
          if (uploadResponse.ok) {
            const uploadData = await uploadResponse.json();
            // Update certificate dengan URL PDF
            await fetch(`/api/certificate/${createdCertificate.id}`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ pdfUrl: uploadData.fileUrl }),
            });
          }
        } catch (uploadError) {
          toast.error("Certificate saved but PDF upload failed");
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-sm text-gray-500">
                    Paste Google Drive link to share with instructure
                  </p>
                </div>
