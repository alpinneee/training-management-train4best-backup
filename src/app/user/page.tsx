        ...
        <div className="flex flex-col gap-2 p-2">
              ...
              <div className="max-h-96 overflow-y-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {users
                      .filter(
                        (user) => !user.role.toLowerCase().includes("instruct")
                      )
                      .map((user) => (
                        <tr key={user.id}>
          ...
          {isDeleteModalOpen && selectedUser && (
            <Modal onClose={() => setIsDeleteModalOpen(false)}>
              ...
              {error &&
                (error.includes("associated records") ||
                  error.includes("Delete anyway?")) && (
                  <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-3 py-2 mt-2 rounded text-xs">
                    <p>
                      Warning: This user has associated records. Force deletion
